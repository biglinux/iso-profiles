...
}
{
    ...
    local *atspi::assert_widget = sub {
        ...
        push @queries, $options{pid};
        return {status => 'passed', complete => 1, widget => {pid => shift @children}};
    };
    calamares->assert_page('launcher-home', 5);
    calamares->assert_page('installer-welcome', 5);
    is_deeply(\@queries, [42, 42], 'GTK and Qt page queries stay inside original launch tree');
