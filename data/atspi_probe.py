...
def _focus_interaction(
    ...
    for _target, component, target_role, target_name in _focus_targets(window):
        if interaction_deadline is not None and time.monotonic() >= interaction_deadline:
            break
        try:
...
def cleanup_new_windows(state_path: Path) -> dict[str, Any]:
...
def main() -> int:
    ...
    try:
        if args.operation == "baseline":
            ...
            result = cleanup_new_windows(args.state)
